...
{
    if (
        filter_var($judgeTask['submitid'], FILTER_VALIDATE_INT) === false ||
...
{
    global $endpoints, $endpointID, $lastrequest, $uwajudge_bit;
    ...
    if ($verb == 'POST' || ($uwajudge_bit && $verb == 'PUT')) {
...
if ($connected) {
    ...
    $judgeCallback = function ($msg) {
        ...
        $expected_uuid_pid = $judgeTask['uuid'] . '_' . (string)getmypid();
...
{
    global $endpoints, $endpointID, $uwajudge_bit;
    $endpoint = &$endpoints[$endpointID];
...
function compile(
    ...
    global $myhost, $EXITCODES, $uwajudge_bit;
    ...
    if ($uwajudge_bit) {
        ...
        logmsg(LOG_ERR, "  📦 Fetched source from database." . $sources);
    ...
    if ($uwajudge_bit) {
        ...
        request($url, 'PUT', [
            ...
            'compile_metadata' => rest_encode_file($workdir . '/compile.meta', false),
...
{
    global $EXITCODES, $myhost, $options, $workdirpath, $exitsignalled, $gracefulexitsignalled, $endpointID, $uwajudge_bit;
    ...
    $testcasedir = $workdir . "/testcase" . sprintf('%05d', $judgeTask['testcase_id']);
    $tcfile = fetchTestcase($workdirpath, $judgeTask['testcase_id'], $judgeTask['judgetaskid'], $judgeTask['testcase_hash'], $judgeTask);
    if ($tcfile === null) {
    ...
    logmsg(LOG_ERR, "Test run command: $test_run_cmd");
...
function fetchTestcase(string $workdirpath, string $testcase_id, int $judgetaskid, string $testcase_hash, array $judgeTask): ?array
{
    global $uwajudge_bit;
