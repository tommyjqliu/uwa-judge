 ...
 */
export default async function throttle(_key: string, maxRequests: number, timeWindow: number) {
    ...
    if (timestamps.length >= maxRequests) {
        throw new Error(`Too many requests. Max limit of ${maxRequests} requests within ${timeWindow} seconds exceeded.`);
    }
