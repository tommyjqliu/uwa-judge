...
const columns: ColumnDef<Problem & { problemVersion: ProblemVersion }>[] = [
    {
        ...
        header: "Problem",
        cell: ({ row }) => {
            return <Link href={`/code?problemId=${row.original.id}`} className="hover:underline">{row.original.problemVersion.name}</Link>
        }
    },
];
    ...
    return (
        ...
        >
            <div className="container">
                ...
                <div className="mb-4">
                    <p><strong>Publish Date:</strong> {assignment.publishDate ? new Date(assignment.publishDate).toLocaleString() : 'Not set'}</p>
