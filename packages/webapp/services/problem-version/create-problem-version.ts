 ...
 * @returns
 */
export const createProblemVersion = (file: File) =>
  withZipFile(file, async (zip) => {
    ...
    const testCases = (["sample", "secret"] as TestCaseType[])
      .map((type) =>
        zip
          .getEntries(`data/${type}`)
          .filter((entry) => entry.endsWith(".in"))
          .map((inputPath) => {
    ...
    assert(
      testCases.filter((i) => i.type === "secret").length > 0,
      "At least one secret test case is required",
    );
    ...
    if (metadata.validation.includes("custom")) {
      ...
      assert(
        filePaths.every((filePath) => path.dirname(filePath) === firstDir),
        "All output validators must be in the same directory",
      );
      outputValidator = await createExecutable(
        `${name}-${hash}`,
        `Validator for Problem ${name}`,
        combinedRunCompare ? "run" : "compare",
        filePaths,
      );
    }
